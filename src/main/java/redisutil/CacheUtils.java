...
public final class CacheUtils {
	...
	static{
		...
		String hostName="10.10.110.35:26379;10.10.110.45:26379;10.10.110.49:26379";
		Set<String> sentinels = new HashSet<String>();
		sentinels.addAll(Arrays.asList(hostName.split(";")));

		jedisPool = new JedisSentinelPool("mymaster", sentinels, jedisPoolConfig, 4000, "online-redis", 0);
	 ...
	 * @return 缓存值
	 */
	public static <T> T get(final Integer dbIndex, final String key, final Class<T> clazz, final boolean useJsonSerialize) {
		if (useJsonSerialize) return JsonUtils.toObject(get(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET)), clazz);
		else return (T) JdkSerializeUtils.deserialize(get(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET)));
	}
	 ...
	 * @param useJsonSerialize true时使用json格式进行反序列化缓存值，false时使用jdk进行反序列化
	 */
	public static void set(final Integer dbIndex, final String key, final Object value, final boolean useJsonSerialize) {
		if (useJsonSerialize) set(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET), JsonUtils.toJsonBytes(value));
		else set(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET), JdkSerializeUtils.serialize(value));
	}
	 ...
	 * @param useJsonSerialize true时使用json格式进行反序列化缓存值，false时使用jdk进行反序列化
	 */
	public static void setEx(final Integer dbIndex, final String key, final Object value, final int seconds, boolean useJsonSerialize) {
		if (useJsonSerialize) setEx(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET), JsonUtils.toJsonBytes(value), seconds);
		else setEx(dbIndex, StringUtils.serialize(key, DEFAULT_CHARSET), JdkSerializeUtils.serialize(value), seconds);
	}
